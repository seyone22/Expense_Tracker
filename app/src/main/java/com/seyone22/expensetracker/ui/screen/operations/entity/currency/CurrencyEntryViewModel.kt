package com.seyone22.expensetracker.ui.screen.operations.entity.currency
...
class CurrencyEntryViewModel(private val currencyFormatsRepository: CurrencyFormatsRepository) :
    ViewModel() {
    ...
    fun updateUiState(currencyDetails: CurrencyDetails) {
        currencyUiState =
            CurrencyUiState(
                ...
                isEntryValid = validateInput(currencyDetails)
            )
    }
...
fun CurrencyFormat.toCurrencyUiState(isEntryValid: Boolean = false): CurrencyUiState =
    CurrencyUiState(
